...
from .simple_interpolator import interpolate_gdp_monthly
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        '--simple',
        action='store_true',
        help='Run simplified interpolation pipeline'
    )
...
def main() -> int:
    ...
    try:
        ...
        # Handle the simple interpolation mode
        if args.simple:
            logger.info("Running simplified interpolation pipeline")
            output_file = args.output
            if output_file and not output_file.endswith('.csv'):
                output_file = str(Path(output_file).with_suffix('.csv'))
            else:
                output_file = 'data/monthly_gdp.csv'
                
            interpolate_gdp_monthly(args.input, output_file, args.debug)
            logger.info(f"Simplified interpolation completed. Results saved to {output_file}")
            return 0
        
        # Regular full pipeline mode
