...
class SplineDetrending:
    ...
    def _fit_cubic_spline(self, data: np.ndarray, nknots: int) -> np.ndarray:
        """
        ...
        if nknots > 0:
            knots = np.linspace(0, n-1, nknots+2)[1:-1]
            ...
            # If nknots = 0, return zeros (no detrending)
            return np.zeros_like(data)
            
    def detrend_quarterly_and_monthly(self, quarterly_data: np.ndarray, nknots: int = 5) -> Tuple[np.ndarray, np.ndarray]:
        """
        Detrend quarterly data and create corresponding monthly detrended data.
        
        This method implements the functionality of cspline_qm2 from the MATLAB code.
        It fits a cubic spline to quarterly data and creates both quarterly and monthly
        detrended series.
        ...
        Returns:
            Tuple of (detrended_quarterly, detrended_monthly)
        """
        # Call the static method directly
        return create_spline_qm2(quarterly_data, nknots)
        
    def detrend_monthly(self, monthly_data: np.ndarray, nknots: int = 4) -> np.ndarray:
        """
        Detrend monthly data using cubic spline.
        
        This method implements the functionality of cspline from the MATLAB code.
        ...
        """
        # Call the static method directly
        return create_spline(monthly_data, nknots)
